...
import numpy as np
from mod_body import mod_body_server, mod_body_ui
...
app_ui = ui.page_sidebar(
    ui.sidebar(
        ui.img(src="coords.png", 
            style="width: 100%; max-width: 225px;", class_="border"),
        ui.accordion(
            ui.accordion_panel("Settings",
            ...
            ui.accordion_panel("Earth",
                mod_body_ui("earth", enable=True, mass=597.216, speed=0.0126, theta=270, phi=90),
            ),
            ui.accordion_panel("Moon",
                mod_body_ui("moon", enable=True, mass=7.347, speed=1.022, theta=60, phi=90),
            ),
            ui.accordion_panel("Planet X",
                mod_body_ui("planetx", enable=True, mass=7.347, speed=1.022, theta=270, phi=60),
            ),
            class_="mt-4",  # mt-4: margin top 4; adds a bit of space above the accordion
            style="--bs-accordion-bg: --bslib-sidebar-bg;", # Give the accordion the same background color as the sidebar
        ),
        position="right",
        open="always",
        max_height_mobile="auto", # In mobile mode, let the sidebar be as tall as it wants
    ),
    ui.div(
        ui.input_action_button("run", "Run simulation", icon=icon_svg("play"), class_="btn-primary")
    ),
...
def server(input, output, session):
    earth_body   = mod_body_server("earth", "Earth", [0, 0, 0])
    moon_body    = mod_body_server("moon", "Moon", [3.84e5, 0, 0])
    planetx_body = mod_body_server("planetx", "Planet X", [-3.84e5, 0, 0])
