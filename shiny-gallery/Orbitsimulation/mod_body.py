...
@module.ui
def mod_body_ui(enable, mass, speed, theta, phi):
    return ui.TagList(
        ui.input_checkbox("enable", "Enable", enable),
        ui.panel_conditional("input.enable",
            ui.input_numeric("mass", "Mass (10^22 kg)",
                mass,
            ),
            ui.input_slider("speed", "Speed (km/s)",
                0, 1, value=speed, step=0.001,
            ),
            ui.input_slider("theta", "Angle (5)", 
                0, 360, value=theta
            ),
            ui.input_slider("phi", "5", 
                0, 180, value=phi
            ),
        ),
    )
...
@module.server
def mod_body_server(input, output, session, label, start_vec):
    @reactive.calc
    def body_result():
        if not input.enable():
            return None
